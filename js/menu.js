window.$menu = function (opts) {
    ...
    observeItems();
    if (Opt.cool) {
        $scroll_wrap.addEventListener('scroll', coolScrollHandler)
        setTimeout(drawPath)          // 放在宏任务队列中，防止找不到DOM
    } else {
        // $scroll_wrap.addEventListener('scroll', simpleScrollHandler)
        window.onscroll = simpleScrollHandler;
    }
    ...
    function simpleScrollHandler(el) {
        ...
        for (let i = allCatalogs.length - 1; i >= 0; i--) {
            if (allCatalogs[i].offsetTop <= document.scrollTop) {
    ...
    function getCatalogsTree(catalogs) {
        ...
        for (let i = 0; i < catalogs.length; i++) {
            ...
            id = 'heading-' + i
            catalogs[i].id = id
            catalogs[i].classList.add("headings");
            treeItem = {
                name: title,
                tagName: tagName,
                id: id,
        ...
        if (tree) {
            ...
            for (let i = 0; i < tree.length; i++) {
                if (isEqual(tree[i].parent, _parent)) {
                    hasChild = true
                    ul += `<li><div class='${Opt.linkClass} cl-level-${tree[i].level} cl-link-${i}' ${Opt.datasetName}='${tree[i].id}'>${tree[i].name}</div>`
